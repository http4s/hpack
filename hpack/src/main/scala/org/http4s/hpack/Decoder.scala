...
final class Decoder(dynamicTable: DynamicTable) {
  ...
  private[this] def insertHeader(headerListener: HeaderListener, name: Array[Byte], value: Array[Byte], indexType: IndexType): Unit = {
  ...
  private[this] def addHeader(headerListener: HeaderListener, name: Array[Byte], value: Array[Byte], sensitive: Boolean): Unit = {
    if (name.length == 0) {
